# GeoWorld

## 전시하기

1. Chrome 등의 브라우저 실행
2. 주소 입력
  ```
  https://ym-geoworld.vercel.app
  ```
3. 관리자 비밀번호 입력
4. F11 눌러서 전체 화면으로 만들기

전시 기간 외에 악용 우려로 **사이트 주소 노출되지 않게 주의**

## 참고한 코드 및 자료

- <https://github.com/benlikescode/GeoHub>
- <https://github.com/GeoGuess/GeoGuess>
- <https://codepen.io/ShadowShahriar/pen/zYPPYrQ>
- <https://codepen.io/hqdrone/pen/vYGJXZN>
- <https://github.com/mihai-craita/countries_center_box>
- <https://github.com/AshKyd/geojson-regions>

## 사용한 리소스

### 폰트

- [Pretendard](https://github.com/orioncactus/pretendard)

### 이미지

- 사이트 아이콘: <https://www.flaticon.com/free-icon/map_1865269?term=map&page=1&position=33&origin=search&related_id=1865269>
